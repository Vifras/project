	...
	<table width="100%" style="margin-left:10px; margin-top:5px;">
		...
		<tr>
			<td colspan="2">
				...
				<?php 
				echo "<a style='cursor:pointer;' title='Lihat Kartu Keluarga' border='0' onclick=\"javascript:window.location.href='index_petugas.php?page=lihat_album&id=$hasil[id]';\">Lihat Album</a>";
				?>
			</td>
		</tr>
	</table>
...
echo "<center>";
if($start !=0){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=album_detail_edit&start=".($start-$data)."'>Prev</a>";
}
...
for ($i=0; $i<$hal; $i++){
	...
	if($start == $x){
		...
		echo "&nbsp;[<a href='index_petugas.php?page=album_detail_edit&start=$x'>".($i+1)."</a>]";
	}
}

//Link untuk Next
if($start != $x){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=album_detail_edit&start=".($start+$data)."'>Next</a>";
}
?>
</div>