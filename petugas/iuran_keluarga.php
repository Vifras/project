...
<hr>
<form method="POST" style="display:inline-block; " action="index_petugas.php?page=iuran_keluarga">
	<?php
...
<table class="table" style="margin-top:-5px">
	...
	if($jumlah == 0){
		...
		while($b = mysql_fetch_array($a)){
			...
			echo "<td align='center'><img src='images/b_browse.png' title='Atur iuran keluarga' border='0' onclick=\"javascript:window.location.href='index_petugas.php?page=lihat_iuran&id=$b[no_kk]';\"></td>";
			echo "</tr>";
			$no++;
		}
	}		
...
echo "<center>";
if($start !=0){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=iuran_keluarga&start=".($start-$data)."'>Prev</a>";
}
...
for ($i=0; $i<$hal; $i++){
	...
	if($start == $x){
		...
		echo "&nbsp;[<a href='index_petugas.php?page=iuran_keluarga&start=$x'>".($i+1)."</a>]";
	}
}

//Link untuk Next
if($start != $x){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=iuran_keluarga&start=".($start+$data)."'>Next</a>";
}
?>
</div>