...
<form method="POST" style="display:inline-block; " action="index_petugas.php?page=kegiatan">
	<?php
...
<form method="POST" action="index_petugas.php?page=kegiatan_proses">
...
<table class="table">
	...
	if($jumlah == 0){
		...
		while($b = mysql_fetch_array($a)){
			...
			echo "<td align='center'><img src='images/b_edit.png' title='Edit' border='0' onclick=\"javascript:window.location.href='index_petugas.php?page=kegiatan_edit&id=$b[id_kegiatan]';\"></td>";
			echo "<td align='center'> <img src='images/trash.png' title='Hapus' border='0' onclick=\"javascript: if(confirm('Data ingin dihapus ?')==true){window.location.href='index_petugas.php?page=kegiatan_proses&id=$b[id_kegiatan]&aksi=hapus';}\"/></td>";
			echo "</tr>";
			$no++;
		}
	}		
...
echo "<center>";
if($start !=0){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=kegiatan&start=".($start-$data)."'>Prev</a>";
}
...
for ($i=0; $i<$hal; $i++){
	...
	if($start == $x){
		...
		echo "&nbsp;[<a href='index_petugas.php?page=kegiatan&start=$x'>".($i+1)."</a>]";
	}
}

//Link untuk Next
if($start != $x){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=kegiatan&start=".($start+$data)."'>Next</a>";
}
?>
</div>