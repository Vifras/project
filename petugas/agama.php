...
<form method="POST" style="display:inline-block; margin-top:10px; " action="index_petugas.php?page=agama">
	<?php
...
<form method="POST" action="index_petugas.php?page=agama_proses" enctype="multipart/form-data">
	<input type="hidden" name="aksi" value="tambah"/>
	<table width="100%" style="margin-left:10px; margin-top:5px;">
		<tr>
			<td style="color:white;">Agama</td>
			<td><input type="text" name="txtagama" required /></td>
		</tr>
			<td colspan="2">
...
<h3 style="padding-bottom: 5px; margin-top: 5px;">Data Agama</h3>
<table class="table" style="margin-top:-1px;">
	...
	$jumlah = mysql_query("SELECT * FROM agama");
	...
	$a = mysql_query("SELECT id_agama, agama FROM agama WHERE agama LIKE '%$katakunci%' ORDER BY agama ASC LIMIT $start, $data");
	...
	if($jumlah == 0){
		...
		while($b = mysql_fetch_array($a)){
			...
			echo "<td align='center'>".str_replace($katakunci, "<b style='color:orange'><i>".$katakunci."</i></b>",$b['agama'])."</td>";
			echo "<td align='center'><img src='images/b_edit.png' title='Edit' border='0' onclick=\"javascript:window.location.href='index_petugas.php?page=agama_edit&id=$b[id_agama]';\"></td>";
			echo "<td align='center'> <img src='images/trash.png' title='Hapus' border='0' onclick=\"javascript: if(confirm('Data ingin dihapus ?')==true){window.location.href='index_petugas.php?page=agama_proses&id=$b[id_agama]&aksi=hapus';}\"/></td>";
			echo "</tr>";
			$no++;
		}
	}		
...
echo "Total Agama = $total Data";
//Link untuk Prev
echo "<center>";
if($start !=0){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=agama&start=".($start-$data)."'>Prev</a>";
}
...
for ($i=0; $i<$hal; $i++){
	...
	if($start == $x){
		...
		echo "&nbsp;[<a href='index_petugas.php?page=agama&start=$x'>".($i+1)."</a>]";
	}
}

//Link untuk Next
if($start != $x){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=agama&start=".($start+$data)."'>Next</a>";
}
?>
</div>