...
<form method="POST" style="display:inline-block; margin-top:10px; " action="index_petugas.php?page=pekerjaan">
	<?php
...
<form method="POST" action="index_petugas.php?page=pekerjaan_proses" enctype="multipart/form-data">
...
<table class="table" style="margin-top:-1px;">
	...
	if($jumlah == 0){
		...
		while($b = mysql_fetch_array($a)){
			...
			echo "<td align='center'><img src='images/b_edit.png' title='Edit' border='0' onclick=\"javascript:window.location.href='index_petugas.php?page=pekerjaan_edit&id=$b[id_pekerjaan]';\"></td>";
			echo "<td align='center'> <img src='images/trash.png' title='Hapus' border='0' onclick=\"javascript: if(confirm('Data ingin dihapus ?')==true){window.location.href='index_petugas.php?page=pekerjaan_proses&id=$b[id_pekerjaan]&aksi=hapus';}\"/></td>";
			echo "</tr>";
			$no++;
		}
	}		
...
echo "<center>";
if($start !=0){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=pekerjaan&start=".($start-$data)."'>Prev</a>";
}
...
for ($i=0; $i<$hal; $i++){
	...
	if($start == $x){
		...
		echo "&nbsp;[<a href='index_petugas.php?page=pekerjaan&start=$x'>".($i+1)."</a>]";
	}
}

//Link untuk Next
if($start != $x){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=pekerjaan&start=".($start+$data)."'>Next</a>";
}
?>
</div>