	...
	<table width="100%" style="margin-left:10px; margin-top:5px;">
		...
		<tr>
			<td colspan="2">
				...
				<a href="index_petugas.php?page=agama">Back</a>
			</td>
		</tr>
	</table>
</form>
...
echo "<center>";
if($start !=0){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=agama_edit&start=".($start-$data)."'>Prev</a>";
}
...
for ($i=0; $i<$hal; $i++){
	...
	if($start == $x){
		...
		echo "&nbsp;[<a href='index_petugas.php?page=agama_edit&start=$x'>".($i+1)."</a>]";
	}
}

//Link untuk Next
if($start != $x){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=agama_edit&start=".($start+$data)."'>Next</a>";
}
?>
</div>