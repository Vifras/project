...
<hr/>
<form action="index_petugas.php?page=_iuran_lunas" method="POST">
<?php
if(isset($_POST['cari'])){
...
echo "<center>";
if($start !=0){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=_iuran_lunas&start=".($start-$data)."'>&laquo;</a>";
}
for ($i=0; $i<$hal; $i++){
	$x = $i * $data;
	if($start == $x){
		...
		echo "&nbsp;[<a href='index_petugas.php?page=_iuran_lunas&start=$x'>".($i+1)."</a>]";
	}
}
if($start != $x){
	echo "&nbsp;<a style='font-size:17px' href='index_petugas.php?page=_iuran_lunas&start=".($start+$data)."'>&raquo;</a>";
}
?>
</div>
