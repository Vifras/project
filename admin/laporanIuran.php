...
switch($_REQUEST['aksi']){
	...
	$bayar		= $_POST['txtbayar'];
		...
		echo "<script>window.location.href='index.php?page=laporanIuran';</script>";
		}else{
	...
	// echo "<meta http-equiv='refresh' content='1;url=index.php?page=laporanIuran'/>";
	// }else{
...
?>
<form method="POST" name="form" action="index.php?page=laporanIuran">
...
<?php
	...
	if($jumlah == 0){
	    ...
	    $tgl_ind=substr($data_date,8,2)."-".substr($data_date,5,2)."-".substr($data_date,0,4);
			...
			// echo "<td align='right'><img src='images/b_browse.png' title='Lihat' border='0' onclick=\"javascript:window.location.href='index.php?page=lihat_laporanIuran&id=$b[id_laporanIuran]';\"></td>";
			//echo "<td align='center'><img src='images/b_edit.png' title='Edit' border='0' onclick=\"javascript:window.location.href='index.php?page=laporanIuran&id=$b[id]';\"></td>";
			echo "<td align='center'> <img src='images/trash.png' title='Hapus' border='0' onclick=\"javascript: if(confirm('Data ingin dihapus ?')==true){window.location.href='index.php?page=laporanIuran&id=$b[id]&aksi=hapus';}\"/></td>";
			echo "</tr>";
			$no++;
		}
	}	
...
echo "<center>";
if($start !=0){
	echo "&nbsp;<a style='font-size:17px' href='index.php?page=laporanIuran&start=".($start-$data)."'>Prev</a>";
}
...
for ($i=0; $i<$hal; $i++){
	...
	if($start == $x){
		...
		echo "&nbsp;[<a href='index.php?page=laporanIuran&start=$x'>".($i+1)."</a>]";
	}
}

if($start != $x){
	echo "&nbsp;<a style='font-size:17px' href='index.php?page=laporanIuran&start=".($start+$data)."'>Next</a>";
}
?>