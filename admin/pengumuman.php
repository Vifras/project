...
<hr>
<form method="POST" style="display:inline-block; " action="index.php?page=pengumuman">
<?php
...
<?php
	...
	if($jumlah == 0){
	    ...
	    $tgl_ind=report_date($data_date);
			...
			echo "<td align='right'><img src='images/b_browse.png' title='Lihat' border='0' onclick=\"javascript:window.location.href='index.php?page=lihat_pengumuman&id=$b[id_pengumuman]';\"></td>";
			echo "<td align='center'><img src='images/b_edit.png' title='Edit' border='0' onclick=\"javascript:window.location.href='index.php?page=pengumuman_edit&id=$b[id_pengumuman]';\"></td>";
			echo "<td align='left'> <img src='images/trash.png' title='Hapus' border='0' onclick=\"javascript: if(confirm('Data ingin dihapus ?')==true){window.location.href='index.php?page=pengumuman_proses&id=$b[id_pengumuman]&aksi=hapus';}\"/></td>";
			echo "</tr>";
			$no++;
		}
	}		
...
echo "<center>";
if($start !=0){
	echo "&nbsp;<a style='font-size:17px' href='index.php?page=pengumuman&start=".($start-$data)."'>Prev</a>";
}
...
for ($i=0; $i<$hal; $i++){
	...
	if($start == $x){
		...
		echo "&nbsp;[<a href='index.php?page=pengumuman&start=$x'>".($i+1)."</a>]";
	}
}

if($start != $x){
	echo "&nbsp;<a style='font-size:17px' href='index.php?page=pengumuman&start=".($start+$data)."'>Next</a>";
}
?>
<br>
<br>
<a href="index.php?page=tambahpengumuman" class="btn-login"> 
