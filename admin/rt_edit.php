...
$nik_rt = $_GET['id'];
$a = mysql_query("SELECT nik_rt, foto, nik_rw, rt, nama, alamat FROM rt WHERE nik_rt = '$nik_rt' ");
...
<form method="POST" action="index.php?page=rt_proses" enctype="multipart/form-data">
	...
	<input type="hidden" name="idx" value="<?php echo $hasil['nik_rt']?>"/>
	<table width="100%" style="margin-left:10px; margin-top:5px;">
		<tr>
			<td style="color:white;">NIK</td>
			<td><input type="text" name="txtnik" onkeyup="javascript:if(isNaN(this.value)){this.value='0'; this.select();}" value="<?php echo $hasil['nik_rt'];?>" required /></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><input type="text" name="txtrt" value="<?php echo $hasil['rt'];?>" required/></td>
		</tr>
		<tr>
		...
		<tr>
			<td colspan="2">
				...
				<a href="index.php?page=rt">Back</a>
			</td>
		</tr>
	</table>
</form>
...
<table class="table">
	...
	if($jumlah == 0){
		...
		while($b = mysql_fetch_array($a)){
			...
			echo "<td align='center'>$b[nik_rt]</td>";
...
echo "<center>";
if($start !=0){
	echo "&nbsp;<a style='font-size:17px' href='index.php?page=rt_edit&start=".($start-$data)."'>Prev</a>";
}
...
for ($i=0; $i<$hal; $i++){
	...
	if($start == $x){
		...
		echo "&nbsp;[<a href='index.php?page=rt_edit&start=$x'>".($i+1)."</a>]";
	}
}

//Link untuk Next
if($start != $x){
	echo "&nbsp;<a style='font-size:17px' href='index.php?page=rt_edit&start=".($start+$data)."'>Next</a>";
}
?>
</div>