...
<form method="POST" style="display:inline-block;" action="index.php?page=surat">
	<?php
...
<table class="table" style="margin-top:-10px;">
	...
	if($jumlah == 0){
		...
		while($b = mysql_fetch_array($a)){
			...
			echo "<td align='center'><img src='images/b_browse.png' title='Lihat' border='0' onclick=\"javascript:window.location.href='index.php?page=lihat_surat&id=$b[id_surat]';\"></td>";
			//echo "<td align='center'>".str_replace($katakunci, "<b style='color:orange'><i>".$katakunci."</i></b>",$b['no_kk'])."</td>";
			echo "</tr>";
			$no++;
		}
...
echo "<center>";
if($start !=0){
	echo "&nbsp;<a style='font-size:17px' href='index.php?page=surat&start=".($start-$data)."'>Prev</a>";
}
...
for ($i=0; $i<$hal; $i++){
	...
	if($start == $x){
		...
		echo "&nbsp;[<a href='index.php?page=surat&start=$x'>".($i+1)."</a>]";
	}
}

//Link untuk Next
if($start != $x){
	echo "&nbsp;<a style='font-size:17px' href='index.php?page=surat&start=".($start+$data)."'>Next</a>";
}
?>
</div>