...
$id_agama = $_GET['id'];
$a = mysql_query("SELECT * FROM agama WHERE id_agama = '$id_agama' ");
...
<!--<form method="POST" style="display:inline-block; " action="index.php?page=agama">
...
echo "<center>";
if($start !=0){
	echo "&nbsp;<a style='font-size:17px' href='index.php?page=agama_edit&start=".($start-$data)."'>Prev</a>";
}
...
for ($i=0; $i<$hal; $i++){
	...
	if($start == $x){
		...
		echo "&nbsp;[<a href='index.php?page=agama_edit&start=$x'>".($i+1)."</a>]";
	}
}

//Link untuk Next
if($start != $x){
	echo "&nbsp;<a style='font-size:17px' href='index.php?page=agama_edit&start=".($start+$data)."'>Next</a>";
}
?>
</div>